...
class Calendar extends Component {
  ...
  changeFocusMonthYear = (month, year) => {
    const newDay = moment()
    newDay.set('M', month-1)
    newDay.set('Y', year)
    newDay.startOf('month')
    const currentMonth = moment(this.state.currentMonth[0])
    currentMonth.startOf('month')
    const subtrast = newDay.diff(currentMonth, 'M')

    this.addMonth(subtrast)
  }
